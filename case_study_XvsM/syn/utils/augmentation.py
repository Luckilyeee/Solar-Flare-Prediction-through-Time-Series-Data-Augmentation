...
def rotation(x):
    # Randomly flip along feature dimension
    flip = np.random.choice([-1, 1], size=(x.shape[0], 1, x.shape[2]))

    # Shuffle the time step indices
    rotate_axis = np.arange(x.shape[1])  # Create index for time steps
    np.random.shuffle(rotate_axis)  # Shuffle time steps

    # Apply flipping and shuffle time steps
...
def random_guided_warp(x, labels, slope_constraint="symmetric", use_window=True, dtw_type="normal", verbose=0):
    ...
    import utils.dtw as dtw  #
    """
    Randomly guided DTW warping augmentation.

    Parameters:
    - x: np.ndarray of shape (num_samples, time_steps, num_features) - Time series dataset
    - labels: np.ndarray of shape (num_samples,) or one-hot encoded labels
    - slope_constraint: str - "symmetric" or "asymmetric" for DTW constraints
    - use_window: bool - Whether to use a DTW window constraint
    - dtw_type: str - "normal" (standard DTW) or "shape" (ShapeDTW)
    - verbose: int - Controls debugging output

    Returns:
    - np.ndarray - Augmented dataset
    """

    np.random.seed()  # Ensure randomness is used properly
    if use_window:
        window = np.ceil(x.shape[1] / 10.).astype(int)
    else:
        window = None

    orig_steps = np.arange(x.shape[1]) + np.random.uniform(-0.5, 0.5, size=x.shape[
        1])  # Introduce randomness in interpolation indices
    ...
    for i, pat in enumerate(tqdm(x)):
        choices = np.delete(np.arange(x.shape[0]), i)
        choices = choices[labels[choices] == labels[i]]  # Filter for same-class samples

        if choices.size > 1:  # Ensure enough diversity
            random_prototype = x[np.random.choice(choices, replace=False)]
            ...
            # Add slight variations to avoid identical results
            jitter_amount = np.random.uniform(-0.01, 0.01, size=warped.shape)
            warped = warped + jitter_amount
            ...
            if verbose > -1:
                print(f"Skipping warping for sample {i}, not enough choices in class {labels[i]}")
            ret[i, :] = pat  # Keep original if no alternative is found
